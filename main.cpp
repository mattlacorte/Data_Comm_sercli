...
int main() {
	...
	char host[NI_MAXHOST]; //client1's remotoe name
	...
	while (true) {
		...
		if (bytesRecieved == 0) { //exit code. 'X' out
			cout << "Client disconnected." << endl;
			break;
		}

		//handling client1 messages
		cout << buff << endl; //repeat message to server
		send(clientSocket, buff, bytesRecieved + 1, 0); //echo message
